...
**145 decision cards** (⟳ 09-09: **IIIF-12 RULED (a″) in Session 7 Step 0 — the register's last open card is closed; every card now carries a final status.** Wire-verified on stage v0.10.0 with two trace predictions overturned and the F7 "bug" withdrawn after a room challenge (findings F1–F7 in `session-7-iiif-port.md`; dumps 10–26); F3 asymmetry commented on #661; ⟳ 09-07: **DIS-19 verification pass completed outside the sprint** — all six single-asset-manifest examples now real platform output; adjuncts IIIF-expression gaps → protagonist **#1299**; same-container transcode collision evidenced on **#970**; ⟳ 08-28: **session 6 complete** — all 27 IIIF+AUTH cards final: 13 IIIF ruled in-room, IIIF-12 ⏸ BLOCKED on iiif-presentation #660/#661, AUTH-01 ruled (stub page), AUTH-02..11 captured as design considerations on protagonist #538, AUTH-12 was already resolved; **125 closed register-wide** (IIIF-12 is the only open card); ⟳ 08-26 session 6: **IIIF-15** minted (operations-table presentation); ⟳ 08-26: **session 5 complete** — all 18 ADJ cards final: 14 ruled in-room + the ADJ-09/ADJ-15 partials finished (ADJ-17/18 were mechanical), **101 closed register-wide**; ⟳ 08-19: **session 4 complete** — its 17 open cards ruled (DIS-19 by
...
| ADJ-16 | Intro example + pipeline narrative aspirational | DOC-WRONG (composite) | doc | ✅ (a) example trimmed to supplied adjuncts; pipelines = planned, dead links removed; text in scratch |
...
| IIIF-12 | "JSON is King" update semantics — verify | DESIGN | doc / rfc | ✅ (a″) RULED session 7, 2026-09-09 — all scenarios wire-verified on stage v0.10.0 (findings F1–F7, session-7 port log; dumps 10–26): matching/ordering ✓, canvas-id customer-global uniqueness NEW, create/update asymmetry (F3 → comment on #661), explicit-choiceOrder Choice recipe (missing-choiceOrder 400 = 0.11 twin, #649), round-trip 400 = documented #661 gotcha, reorder-via-items recipe ✓ (F7 "bug" withdrawn after room challenge — public ids stable; items-ids rule documented). LAST OPEN CARD — register complete |
