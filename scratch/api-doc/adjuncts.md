...
## Intro example second member + pipeline narrative — ⏸ REPLACED 2026-08-26 (hygiene session 5, ADJ-16 ruled (a))

> The headline collection example's second adjunct was a *pipeline-generated* AnnotationPage
> (`creator: "pipeline:annotationsFromOCR/v0.9.1"`, `source` → the ALTO adjunct, no `origin`). It is
> now a caller-supplied AnnotationPage via `origin`; the narrative and Aside describe supplied
> adjuncts only and say pipelines are planned (no link — `../pipelines` is unported, PRO-09).
> The removed lines/paragraph (design intent, restore with the private-protagonist #13 epic):
...
> "The second example does not have an `origin` but does have a `creator` with the value
> `"pipeline:annotationsFromOCR"`. This second adjunct was _made by the platform itself_ from the
> first adjunct (specified by the `source` field), using the _pipeline_ specified by the `creator`
> field." — Aside: "This page does not discuss pipelines that create adjuncts. From here on, all
> adjuncts mentioned are assumed to be created by the API caller. For adjuncts generated by the
> platform, see [pipelines](../pipelines)." — field-usage intro: "See [pipelines](../pipelines) for
> further clarification of how these fields are used when invoking a platform pipeline."

## IIIF-expression claims corrected to wire reality — DIS-19 verification pass, 2026-09-07
...
- `fileSize` (IIIF P4) computed from `size` on seeAlso/annotations/rendering entries and inline
  bodies (example showed `"fileSize": 36032 // computed` etc.; field-usage table said
  "yes, as IIIF P4 property fileSize").
- `language` on the direct `annotations` AnnotationPage link (only `id`/`type`/`label` are emitted).
- `motivation` normalised to an array ("will always appear ... as an array of motivation values");
  actual output is the supplied string, scalar. Inline annotation `body` and `target` were also shown
  as arrays; actual output is a single object / string.

Verification evidence: `scratch/hygiene-sprint/dis-19-manifests/put-example-1-rusty-boat.json`.
Code: `Orchestrator/Infrastructure/IIIF/Manifests/ManifestV3Builder.cs` (`CreateExternalResource`,
`CreateInlineAnnotation`, `IIIFLinkType.Annotations` case).
