...
When the Asset Selection modal is open, if you close it or click outside while assets are selected, those assets are retained in a **Pending Assets** list and can be added later.

![Fig. 3.2.2d: Pending assets](../../../assets/portal/portal_026.png)

### Canvas Label Settings

By default (and illustrated in the previous section), assets managed in your Spaces are labelled with their Asset ID (filename), as default. However, there may be scenarios where you want to use one of the metadata values; string 1, string 2 or string 3 which are set on the Asset(s) in your Spaces, as the value for the Canvas label(s) instead.
...
Now you will see the selected metadata field values being used as the Canvas labels, when you add those assets to your Manifest. If the selected metadata field is empty for one or more of the assets selected, the Asset ID will be populated by default as the Canvas label. Once you have saved your Manifest using the selected value for the Canvas labels, you can alter your Canvas label setting as needed.
...
You can reorder and delete Manifest content directly in the portal Manifest view.
...
Using the checkboxes to the left of the item to select multiple items, you can drag them together. The labels of the selected items are shown while dragging, along with a red line indicating the drop position.

![Fig. 3.2.3b: Dragging multiple rows](../../../assets/portal/portal_033.png)

### The three-dots menu

Each item has a **⋮** (vertical ellipsis) menu offering **Delete**, **Move to start**, and **Move to end** options.

![Fig. 3.2.3c: Individual row options](../../../assets/portal/portal_031.png)
...
![Fig. 3.2.3d: Confirm or cancel deletion](../../../assets/portal/portal_032.png)
...
- Click the **i** icon to view the Manifest summary.
