...
```json copy
{
  ...
  "@id": "https://api.dlcs.example/customers/99/spaces/5/images/b2921371x_0001.jp2",
  "@type": "vocab:Image",
  ...
  "batch": "https://api.dlcs.example/customers/99/queue/batches/871143",
  "ingesting": false,
  ...
  "deliveryChannels": [
    ...
    },
    {
      "@type": "vocab:DeliveryChannel",
      "channel": "thumbs",
      "policy": "https://api.dlcs.example/customers/99/deliveryChannelPolicies/thumbs/default"
    },
    {
      "@type": "vocab:DeliveryChannel",
      "channel": "file",
  ...
  "metadata": "https://api.dlcs.example/customers/99/spaces/5/images/b2921371x_0001.jp2/metadata",
  "storage": "https://api.dlcs.example/customers/99/spaces/5/images/b2921371x_0001.jp2/storage"
}
```
...
* The [space](../asset#space) property indicated what [Space](../space) the asset lives in. Spaces are used to organise assets.
...
* The `manifests` property lists any [IIIF Presentation Manifests](../iiif-manifests) this asset is used in (empty here, as it was just registered).
...
An asset can be registered via the API _as an API resource, like the above_ in one of three ways:

 - HTTP PUT an asset API object to its API address directly
 - Include the asset as a member of a Collection that is HTTP Posted to a [Queue](../queues)
 - Include the asset when creating or updating a IIIF Manifest

The first is more common for ad hoc, one-at-a-time asset registration. The second is recommended for systems integration and other workflows. As a customer, you have a [Queue](../customer#queue) that you POST [Hydra Collections](../collections) of assets to. 
...
Before you can register an asset, you need to create a Space for it to live in. If we assume Space 37 already exists, at a minimum, a registration for an image (or any other type of asset) looks like this:
...
The PUT address is the API resource address of the asset. PUT is used for both creation and updating; here, assume we are creating the asset for the first time. (A PUT to an *existing* asset always causes it to be re-fetched from origin and re-processed - see [Reprocessing](../reprocessing).) The last path element of the PUT address looks like a filename in this example, but it can be any string that's a valid URL path element.
...
{
    // ...   
    "imageService": "https://dlcs.example/iiif-img/99/37/my-image.tiff",
    // ...
}
```

This is the public endpoint for deep zoom and arbitrary region requests. If you request it directly, you get the IIIF Image Information document (the info.json). You can open the service in a viewer like this:
...
<LinkCard
  title="💻 HTTP POST to Queue"
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p02_registering/post_to_queue.py"
  description="Registering an asset by POSTing to a queue"
/>

Here, we needed to supply an additional field of the asset - its [Space](../space). The queue can take assets destined for any space, even in the same PST payload.
...
The HTTP response is a `201 Created` status, and the response body is a [Batch](../batch) - a submitted job of assets.
...
In both the above cases, the API JSON object returned from the POST to the queue is a [Batch](../batch). A batch represents a single submitted job to the platform. On your side, the concept of a job might involve multiple batches. The platform limits the number of assets in a single Collection POSTed to the queue to 250 (a platform-configured limit), so if the above example had been the images of a 600 page book (the job on your side), you would need to make three POSTs (e.g., of 250, 250 and 100 assets).

You can use batches to keep track of submitted assets as the platform is working on them. In the linked code example, we poll the batch until the `completed` count is 3. Other linking properties from the batch can be used to examine the state of the registered assets. Each asset also links back to the _most recent_ batch it was a part of.

<LinkCard
  title="💻 Multiple assets to Queue"
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p02_registering/queue_multiple_assets.py"
  description="Send three assets to the queue in a single job, and monitor the batch to watch them complete."
/>
