...
| `assetOrder` | The metadata field(s) used to order assets in the projection (canvases, PDF pages, zip entries). Can take multiple fields and `asc`/`desc` modifiers — see below | `&assetOrder=n2` |
...
| `manifest` | Select assets that are part of a stored [IIIF Manifest](../iiif-manifests) — the value is the stored manifest's flat id, as minted by the IIIF Presentation API | `&manifest=p1` |
