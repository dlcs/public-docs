...
The platform acts as a hierarchical IIIF repository — like a file system, with Storage Collections acting as directories, and Manifests and IIIF Collections acting as files. Only Storage Collections can contain other resources (see the [IIIF Collections](../iiif-collections) page).
...
\* A creating PUT returns `201` for Manifests, currently `200` for Collections — and `202 Accepted` for a Manifest that registers new assets, with ingest still in progress (see [IIIF Manifests](../iiif-manifests)).
