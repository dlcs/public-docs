...
|:--|:--|:--|:--|:--|:--|:--|
| GET | hierarchical `/99/{path}` | no | — | public IIIF view | 200 | 404 |
| GET | flat `…/manifests/{id}` · `…/collections/{id}` | no | — | redirect to the hierarchical URL | 303 | 404 |
| GET | hierarchical | yes | — | redirect to the flat URL | 303 | 404 |
| GET | flat | yes | `If-None-Match` optional | API view, with `ETag` | 200, 304 | 401, 404 |
| GET | `…/collections/root/search?label=` | yes | — | search across your resources | 200 | 400, 401 |
| POST | flat container `…/collections` · `…/manifests` | yes | — | create; the platform mints the flat id, returned in `Location` | 201 | 400, 401, 403, 409 |
| PUT | flat, new id | yes | must be **absent** | create at your chosen id | 201 (Manifest), 200 (Collection) | 400, 401, 403, 409, 412 |
| PUT | flat, existing id | yes | **required** | replace the resource — including moves and renames | 200 | 400, 401, 403, 412 |
| DELETE | flat | yes | **required** | delete the resource | 204 | 400, 401, 403, 404, 412 |
