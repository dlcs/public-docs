...
Two responses are leaner than the rest: a `404 Not Found` carries only `title`, `instance` and `status` (no `type` or `detail`), and a write request without credentials returns a `401` with an empty body.
