---
title: Asset Query Syntax
sidebar:
  label: Asset Query Syntax
  order: 15
---

import { Aside, LinkCard } from '@astrojs/starlight/components';

<Aside type="caution">
  These features are still under development. Queries based on `string1`, `string2`, `string3`, `number1`, `number2` and `number3` are supported, as are [ordering](#ordering) and [include](#include-children). `tags`, `roles`, `id` queries and multiple values are not yet supported.
</Aside>

Any hyperlink to a collection of assets can accept query parameters that filter the returned assets.

## Applicable endpoints

 - `/customers/{customer}/allImages`
 - `/customers/{customer}/spaces/{space}/images`
 - `/customers/{customer}/queue/batches/{batchId}/images`
 - `/customers/{customer}/queue/batches/{batchId}/assets`

Once you have registered images with the platform you can query it for information on them. One immediate use of this is to monitor the progress of ongoing registrations. For example, if you registered a batch of 100,000 images it will take some time for the platform to process them, and you would like to be able to monitor progress and view any errors that were encountered. You can also query for usage statistics.

## Permitted queries

### Shortcut metadata fields

The 6 built-in string and number fields are supported, plus `manifests`.

```
?string1=my-value
?number1=99
?string1=my-value&number1=99
```

The `manifests` filter matches assets painted in a stored [IIIF Manifest](../iiif-manifests), by the manifest's flat id. It is the only filter that accepts **multiple values** — comma-separated, matching assets that appear in *any* of the named manifests:

```
?manifests=gb799m5z
?manifests=gb799m5z,ab123cd4
```

### Query object

```
?q={"string1":"my-value"}
?q={"number1":99}
?q={"string1":"my-value","number1":99}
?q={"manifests":["gb799m5z"]}
```

### Pagination

```
?pageSize=10&page=2
?q={"string1":"my-value","number1":99}&pageSize=10&page=2
```

### Ordering

You can order results by the value of a field of an asset:

```
?orderBy=height
?orderByDescending=width
```

The field can be one of the metadata shortcut names above (`string1`, `number2`, ...) or one of the following asset properties: `created`, `width`, `height`, `duration`, `mediaType`, `origin`, `tags`, `finished`. Field names are matched case-insensitively. Other field names are not supported and result in an error. If no ordering is provided, results are ordered by `created`.

### Include Children

The only accepted value is `adjuncts`.

```
&include=adjuncts
```

<LinkCard
  title="💻 Query assets by metadata fields"
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p15_asset_queries/asset_queries.py"
  description="Filter assets using shortcut query parameters and the q object, with pagination"
/>
