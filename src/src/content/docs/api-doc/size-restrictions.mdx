...
<Aside type="note">
When an image has roles but `openFullMax` is not set, no thumbnails are produced regardless of the thumbnail policy. The `thumbs` delivery channel only serves content that is accessible without authentication.
</Aside>
