...
<Aside type="note">
Some adjunct properties are not yet expressed in the generated IIIF: `provides` and `size` (intended to appear as the IIIF Presentation 4 `fileSize` property) are stored and returned by the API but do not appear in the output, and an `annotations` page link carries only `id`, `type` and `label`. This is tracked in [protagonist #1299](https://github.com/dlcs/protagonist/issues/1299).
</Aside>
...
Only used when `iiifLink` is `"inlineAnnotation"`. It is intended to appear as the `provides` property of the annotation, but is **not currently expressed** in the generated IIIF — the value is stored and returned by the adjuncts API only ([protagonist #1299](https://github.com/dlcs/protagonist/issues/1299)).
...
| `size`      | recommended   | recommended  | ignored          | ignored         | not currently ([#1299](https://github.com/dlcs/protagonist/issues/1299)) |
| `iiifLink`  | required      | required     | required         | required        | as linking property                  |
| `motivation`| nb**          | nb**         | nb**             | nb**            | as annotation property               |
| `provides`  | nb**          | nb**         | nb**             | nb**            | not currently ([#1299](https://github.com/dlcs/protagonist/issues/1299)) |
