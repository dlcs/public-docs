...
<Aside type="note">
Some adjunct properties are not yet expressed in the generated IIIF. `provides` and `size` (which will appear as the IIIF Presentation 4 `fileSize` property) are collected and returned by the API now, but emitting them waits until the platform implements Presentation 4 — planned, tracked in [protagonist #1299](https://github.com/dlcs/protagonist/issues/1299). The same issue notes that an `annotations` page link currently carries only `id`, `type` and `label`.
</Aside>
...
Only used when `iiifLink` is `"inlineAnnotation"`. It will appear as the `provides` property of the annotation, but is **not yet expressed** in the generated IIIF — the value is collected and returned by the adjuncts API now, and will be emitted when the platform implements IIIF Presentation 4 ([protagonist #1299](https://github.com/dlcs/protagonist/issues/1299)).
...
| `language`  | optional      | optional     | optional         | optional        | yes (but not on `annotations` page links) |
...
| `size`      | recommended   | recommended  | ignored          | ignored         | as IIIF P4 `fileSize` when Presentation 4 lands ([#1299](https://github.com/dlcs/protagonist/issues/1299)) |
| `iiifLink`  | required      | required     | required         | required        | as linking property                  |
| `motivation`| nb**          | nb**         | nb**             | nb**            | as annotation property               |
| `provides`  | nb**          | nb**         | nb**             | nb**            | when Presentation 4 lands ([#1299](https://github.com/dlcs/protagonist/issues/1299)) |
