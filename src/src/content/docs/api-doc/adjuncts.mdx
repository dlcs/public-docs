...
<Aside type="note">
  All adjuncts on this page are created by the API caller, either by supplying an `origin` for the platform to fetch, or an `externalId` for a resource hosted elsewhere. Platform [pipelines](../pipelines) that generate adjuncts (for example, deriving annotations from OCR) are a planned capability and are not yet available.
</Aside>
