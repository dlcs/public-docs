...
| PATCH | Edit a customer record | vocab:Customer | vocab:Customer | 200 OK, 400 Bad request, 404 Not Found |

Only the `displayName` field is editable, via PATCH. The `name` is set when the customer is created and cannot be changed afterwards.
...
| domain | range | readonly | writeonly |
|:---|:---|:---|:---|
| vocab:Customer | 🔗 hydra:Collection | False | False |
...
<LinkCard
  title="💻 Asset queries on allImages"
...
| [optimised](../origin-strategy#optimised) | OPTIONAL |
...
<LinkCard
  title="💻 Delivery Channel Policies"
...
{
    "channel": "iiif-av",
    "policy": "https://api.dlcs.example/customers/2/deliveryChannelPolicies/iiif-av/default-video",
    "mediaType": "video/mp4"
}
```
...
Collection of all the Named Queries you have configured (plus those provided ‘out of the box’). See the [Named Query](../named-queries) topic for further information.
...
| POST | Submit a [collection](../collections) of assets and get a batch back. | 🔗 hydra:Collection (of vocab:NamedImageQuery) | vocab:Batch | 201 Job has been accepted - Batch created and returned |
...
Collection of user accounts that can log into the portal. See [Portal Users](../portal-users)
for the resource fields and full lifecycle (create, retrieve, update, delete).
...
<Aside type="caution">
Portal user accounts will be deprecated — the current customer portal manages its users
externally. See [Portal Users](../portal-users).
</Aside>
...
| Method | Label | Expects | Returns | Status |
|:---|:---|:---|:---|:---|
| GET | Returns the customer storage resource | - | vocab:CustomerStorage | 200 OK, 404 Not Found |


<LinkCard
  title="💻 Storage"
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p05_customer/storage.py"
...
Api keys allocated to this customer. The accompanying secret is only available at creation time. To obtain a key and a secret, make an empty POST to this collection while authenticated as this customer, and the returned Key object will include the generated secret.
...
<LinkCard
  title="💻 API Keys"
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p05_customer/keys.py"
...
<LinkCard
  ...
  href="https://github.com/dlcs/public-docs/blob/main/dlcs-docs-client/p05_customer/custom_headers.py"
  description="List current custom header configuration and add more."
/>