...
<Aside type="note">
Assets processed before delivery channels existed may include legacy `imageOptimisationPolicy` and `thumbnailPolicy` links in their responses, recording how they were processed at the time. These properties are deprecated and cannot be set — supplying either returns 400 Bad Request; use `deliveryChannels` instead.
</Aside>


## roles

An array of [Role](../access-control#roles) URIs. The roles a user needs to have to access the output of a delivery channel for this asset. A user acquires roles by interacting with an access service — see [Access Control](../access-control).

Roles are always URIs of the form `{...}/customers/{customer}/roles/{roleId}`, i.e., members of the customer's roles collection (not yet manageable through the API — see [Access Control](../access-control)).
...
| Method | Label | Expects | Returns | Status |
|:---|:---|:---|:---|:---|
| GET | Lists all the adjuncts associated with the Asset | - | 🔗 hydra:Collection (of adjuncts) | 200 OK, 404 Not Found |
| POST | Add one or more adjuncts, supplying an `origin` or an `externalId` for each | an adjunct, or a hydra:Collection of adjuncts* | 🔗 hydra:Collection (of the adjuncts)* | 201 Created, 200 OK, 400 Bad Request, 409 Conflict |
