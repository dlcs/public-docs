...
With a PUT, you can supply any or all of the fields [regex](#regex), [strategy](#strategy), [optimised](#optimised), and [order](#order). To set or change the stored [credentials](#credentials) you must PUT a _full_ strategy object — all of `regex`, `strategy`, `credentials`, `optimised` and `order` must be present — and the platform stores the supplied credentials securely. Credentials can only be set when the strategy is `basic-http-authentication` or `sftp`; supplying them for any other strategy type is rejected. Setting the strategy to `basic-http-authentication` requires credentials, and changing the strategy away from it clears any stored credentials.
...
The `credentials` property is required (in a POST to the parent collection, or in a PUT) for the `basic-http-authentication` and `sftp` strategies, and is not permitted for any other strategy — supplying it with `s3-ambient`, for example, is rejected with 400 Bad request. It will never be rendered back via the API — it will always appear as `xxx`.
